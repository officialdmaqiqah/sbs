-- Migration for Phase 3C: Supplier Payment

-- 1. Add journal_entry_id to supplier_payments if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'supplier_payments' AND column_name = 'journal_entry_id'
    ) THEN
        ALTER TABLE supplier_payments ADD COLUMN journal_entry_id UUID REFERENCES journal_entries(id);
    END IF;
END $$;

-- 2. Create the RPC pay_supplier_bill
...
BEGIN
    ...
    FROM user_roles 
    WHERE user_id = p_created_by AND organization_id = p_organization_id;
    
    IF v_user_role NOT IN ('CEO_ADMIN', 'FINANCE') THEN
    ...
    -- AP Account
    SELECT id INTO v_ap_account_id FROM accounts 
    WHERE organization_id = p_organization_id AND account_type = 'Account Payable' AND is_active = true LIMIT 1;
    ...
    INSERT INTO journal_entries (
        ...
        'Supplier Payment: ' || p_payment_number || ' for Bill ' || v_bill_number, 'SupplierPayment', v_payment_id,
