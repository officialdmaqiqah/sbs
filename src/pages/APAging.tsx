...
import { useAPAging } from '../hooks/useFinance';

export default function APAging() {
  const [reportDate, setReportDate] = useState(new Date().toISOString().split('T')[0]);
  const { data: agingData, loading } = useAPAging(reportDate);
  ...
  const exportCsv = () => {
    const csv = ['Bill No,Supplier,Due Date,Outstanding,Days Overdue,Bucket\n'];
    agingData.forEach((a: any) => {
      csv.push(`${a.bill_number},${a.supplier_name || a.supplier_id},${a.due_date},${a.outstanding_amount},${a.days_overdue},${a.bucket}\n`);
    });
    const blob = new Blob([csv.join('')], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `ap_aging_${reportDate}.csv`;
    a.click();
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Accounts Payable Aging</h1>
          <p className="text-slate-500 mt-1">Track overdue supplier bills by age bucket.</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Bill Date</th>
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-6 py-8 text-center text-slate-500">No outstanding bills.</td></tr>
            ) : (
              agingData.map((a: any) => (
                <tr key={a.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900 font-medium">{a.supplier_id}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{a.bill_number}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{a.bill_date}</td>
