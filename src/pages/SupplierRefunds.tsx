...
import { useSupplierRefunds, useCashBankAccounts } from '../hooks/useFinance';

export default function SupplierRefunds() {
  const { data: refunds, refetch: refetchRefunds } = useSupplierRefunds();
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    try {
      arApService.receiveSupplierRefund(formData, 'Admin');
      setIsModalOpen(false);
      refetchRefunds();
      toast.success('Refund received and posted successfully!');
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Supplier Refunds</h1>
          <p className="text-slate-500 mt-1">Record refunds received from suppliers for purchase returns.</p>
        </div>
        <button className="px-4 py-2 border rounded" onClick={() => {
          setFormData({ supplier_id: '', return_id: '', project_id: '', date: new Date().toISOString().split('T')[0], cash_bank_account_id: '', amount: 0, reason: '' });
          setIsModalOpen(true);
        }}>Receive Refund</button>
      </div>
            ...
            {refunds.map(ref => {
              ...
              return (
                <tr key={ref.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{ref.supplier_id}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-lg">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Receive Supplier Refund</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label>Supplier ID / Name</label>
                  <input className="w-full rounded border px-3 py-2" value={formData.supplier_id} onChange={e => setFormData({...formData, supplier_id: e.target.value})} required placeholder="e.g. SUPP-001" />
                </div>
                <div className="space-y-1.5">
                  <label>Date</label>
              ...
              <div className="flex justify-end gap-3 pt-6 border-t mt-6">
                ...
                <button className="px-4 py-2 border rounded bg-brand-600 text-white" type="submit">Receive Refund</button>
              </div>
            </form>
          </div>
        </div>
