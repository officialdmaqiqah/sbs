  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Produksi Kandang</h2>
          <p className="text-sm text-slate-500">Konversi bahan kandang menjadi kandang siap jual</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-200">
            ...
            {!loading && filteredProductions.length === 0 && (
              <tr><td colSpan={6} className="text-center py-8 text-slate-500">Tidak ada riwayat produksi.</td></tr>
            )}
          </tbody>
        </table>
      </div>

      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Draft Produksi Kandang">
        <form onSubmit={handleCreate} className="space-y-4 max-h-[80vh] overflow-y-auto px-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={batchName} onChange={e => setBatchName(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" placeholder="Cth: Kandang Kayu Batch 1" />
            </div>
          </div>
